...
# vendors byte-identical copies so the Go parity test (and any future embed)
...
# The declared set of vendored contracts, by basename under contracts/storage/
# on both sides. Add a name here when a contract is vendored; the loop below
# fails loud if a declared name is missing from EITHER the canon or this repo,
...
readonly CANON_PROBE="$CANON_DIR/contracts/storage/${CONTRACTS[0]}"
...
for name in "${CONTRACTS[@]}"; do
  canon="$CANON_DIR/contracts/storage/$name"
  vendored="contracts/storage/$name"
  if [ ! -f "$canon" ]; then
