<!-- SPDX-License-Identifier: FSL-1.1-Apache-2.0 -->
<!-- Copyright (c) 2025 Open Computer Use Contributors -->

# ROADMAP — ocu-filestore

The tracked phase tracker for the storage service. Every row is cross-checked
against `git log` on the branch it lands on; nothing is marked DONE that the
history does not show. The architecture repo's ADRs are the source of truth for
any BLOCKED row — a blocked phase unblocks only when its ADR leaves
`status: proposed`. This file restates delivery state in this project's own
words; it does not re-decide anything an ADR already pinned.

Status vocabulary: **DONE** (landed + ratified), **BLOCKED** (gated on a named
ADR still `proposed`), **TBD** (contract surface not yet pinned),
**FROZEN** (wire surface pinned, no further change without a contract bump),
**DEFERRED-VERIFICATION** (built; final proof deferred to a named rig).

---

## Phases

| Phase | Scope | Status | Gate / anchor | Evidence |
| ----- | ----- | ------ | ------------- | -------- |
| REST pivot | Retire the broker north/south-face + fixed-proxy lane; object-store service over HTTPS / REST-JSON / JWT; delete dead Connect/unix/peer-cred/STS/lane, add the TLS server | **DONE** | component-04 + ADR-0014 transport model | `69907e6` (delete dead transport + TLS server, wave 5), `ec57b64` (README/docs/CHANGELOG synced, pivot phase-end), `8c6f9ca` (e2e-linux rewritten to REST/TLS) |
| Files-API handle-store | Durable file_id handle store: store interface + record + sentinels, fsync'd durable-append store, scope-bound tombstone Delete + last-write-wins replay, scope-binding resolver Get (keystone), List opaque-cursor paging, `--handle-store` daemon wiring | **DONE** (ratified, store + keystone + Delete + List + cursor + daemon + 3 follow-ups) | ADR-0023 | `a32b5d1` (store iface + sentinels), `19e3e61` (durable-append store), `ef48e24` (tombstone Delete), `1c3d023` (keystone resolver Get, anti-enumeration), `6744d48` (List cursor paging), `7c6d2b0` (`--handle-store` flag + flock + compose + readyz); follow-ups `365e4f7`, `53130bc`, `e1632ae` |
| F9 north Files-API listener | The north listener that makes the durable handle-store reachable on the wire: a dedicated north TLS listener (Mount B) on `--north-bind` — a separate bind from the south `--south-bind`, reusing the south cert — fanned out by a dual-server, serving `/v1/files` and resolving the durable handle-store. Six routes live, split by AUTHORIZATION AXIS rather than by plane: four are read-class — metadata (`GET /v1/files/{file_id}`), list (`GET /v1/files`), content (`GET /v1/files/{file_id}/content`) and the additive archive OCU-extension (`GET /v1/files/archive`), each resolving `intent=read` — and two are WRITE-class: create (`POST /v1/files`) and delete (`DELETE /v1/files/{file_id}`), each resolving `intent=write`, a create being a content mutation and a delete a namespace mutation (the same split the closed south route-op map gives `createFile` and `removeFile`). Create is no longer 501-fenced: the frozen contract pins its `multipart/form-data` upload body and the write path landed at `907b97f`. Scope is the host-attested `filesystem_id` read from the request header on the trusted intra-deployment F9 channel (a wire `fs_id` is a cross-check hint, never authority). Live only when `--handle-store` is set. | **DONE** on ADR-0023 (north contract, `accepted`): the Mount B listener, keystone resolver, and the served verbs are built + green + keystone-mutation-proven on `feat/f9-storage-seam` @ `ffb409f`. The scope-field transport binding rides ADR-0025 (`status: proposed`, architect-agreed shape, owner-merge-pending) — built and fenced in tree, not yet canon-ratified on `origin/next/v1`. | ADR-0023 (north contract, `accepted`); ADR-0025 (F9 scope-field transport, `status: proposed`, architect-agreed shape, owner-merge-gated) | `fe5e533` (shared `internal/denywire` deny mapping; south delegates), `ab33eb4` (`internal/filesapi` read/delete handler + keystone + fenced create), `240d89f` (Mount B north TLS listener on `--north-bind` + dual-server fan-out + main wiring), `deb09b6` (keystone / audit-before-ack / mount-parity / north e2e tests), `ffb409f` (scope-source comment reframe). Keystone (cross-scope and unknown `file_id` → byte-identical 404, no 403 branch) is mutation-proven THROUGH the HTTP layer; coverage floor 89.0% measured firsthand in a Linux + S3 container (≥ 86.0% floor). CI never ran on this branch (`go.yml` triggers only on push/PR-to-`main`; this is a no-PR feature branch under delegation), so the firsthand Linux + S3 run is the authoritative floor check until the filestore PR opens. **Fleet north-activation DONE + LIVE-WIRE-PROVEN:** `deploy/docker-compose.fleet.yml` @ `ec4e301` activates the north listener in the stock fleet topology — webui dials `filestore:7080` directly (F9 keystone, no credential), edge dials `filestore:8444` (TLS, RFC-8693 cred injection); the default loopback `-north-bind 127.0.0.1:7080` is pinned to `0.0.0.0:7080` so the bind is reachable cross-container. Firsthand smoke proved both faces live: north `:7080` returns the keystone-404 (the byte-identical not_found on an unknown `file_id`, no 403 branch) and south `:8444` answers on the mount RPC. The fleet compose-fragment is a delivered filestore-side input (it ships in this tree as `deploy/docker-compose.fleet.yml`); the earlier "fleet compose is ocu-deploy's job" framing is superseded — the activation fragment lands here. **Create write-plane LANDED — supersedes the earlier 501-fence framing:** the frozen contract pins the createFile `multipart/form-data` upload body (`contracts/openapi/files-api.openapi.yaml`, `operationId: createFile`), so createFile left CONSTITUTION #11's fenced set and the write path shipped at `907b97f`. `internal/filesapi/create.go` carries the whole-object ceiling, the scope cross-check, the path canonicalization, `Resolve(intent=write)`, audit-before-ack, and the over/under-declaration aborts, mirroring `internal/southface/upload_multipart.go` as planned. `internal/filesapi/create_fenced.go` and its `TestCreateIsFenced501` guard no longer exist; CONSTITUTION #11's "Enforced" line and its Aspirational entry still name both files and are therefore stale — tracked separately, not corrected in this row. **Authorization-axis caveat, recorded not fixed:** the F9 `ScopeSource` stamps a FIXED `[read]` grant (`internal/filesapi/scopesource.go:fencedGrantedIntents`) and the two write-class verbs add `IntentWrite` to their own evidence, so under every shipped wiring the Resolver has no axis left to fail on: the resolver-error deny arms in `internal/filesapi` are defence in depth against a future scope source, not refusals a deployment can produce. What refuses live on this plane is the ops/s ceiling, the keystone 404, and the downloadable egress gate (which reads a resolved value, not a resolver error). Pinned by `internal/filesapi/resolver_reachability_test.go`. Whether the F9 grant should instead carry the true per-verb intent set is an open owner question. |
| A5-credscope | Service receives only the edge-injected real credential on `Authorization: Bearer`, forwards unmodified; scope check at the service/route layer, engine keyed on the scope string (no JWKS verify of the guest JWT) | **BLOCKED** (GATE-2) | ADR-0019 (`status: proposed`) | Ledger id `A5-credscope` in `docs/pending-phase7.md`; `PENDING-PHASE-7(A5-credscope)` markers stay in tree (no marker flipped) |
| engine-leg-egress | Whether the S3 engine's OWN backend leg retains an egress proxy (ADR-0011-vs-new-model reconciliation); interim engine backend client is a plain strict-TLS dial, never `http.ProxyFromEnvironment` | **BLOCKED** (GATE-3) | ADR-0021 (`status: proposed`) | Ledger id `engine-leg-egress` in `docs/pending-phase7.md`; `PENDING-PHASE-7(engine-leg-egress)` markers stay in tree (no marker flipped) |
| Message bodies | Per-operation request/response body shapes beyond the five frozen wire ids | **TBD** | `x-ocu-tbd-bodies` (awaiting field-level pin in the frozen contract) | Bodies marked TBD in the vendored contract stay TBD until the contract pins them — never invented here |
| Wire shape (5 ids) | The five wire-shape ids (`A1-route`, `A2-multipart`, `A2-octet`, `A3-deny`, `A4-fsid-toplevel`) | **FROZEN** @ `a030b7be914b` (#292) | #292 merged; A4 contradiction resolved | `7ddac98` (freeze the 5 wire-shape ids @ `a030b7be914b`, #292 merged) |
| Container-leg e2e | Live REST/TLS end-to-end over a real container runtime | **DEFERRED-VERIFICATION** (Lima) — coverage leg now run firsthand in a Linux container | — | REST/TLS e2e harness present (`8c6f9ca`). The Linux + S3 coverage leg ran firsthand in a `golang:1.26` container against a MinIO rig (`engine_s3.go` actually covered, 89.0% over `./internal/...`), so the Linux-build + S3-engine surface is verified in a real container. The remaining deferral is narrowly the live-socket deploy-assembly run, still pending on the Lima rig — the coverage container does not stand up the assembled REST/TLS socket end-to-end. |

---

## Follow-ups (tracked, not yet scheduled)

- **Raise the mutation floor.** The mutation gate is now ARMED via
  go-mutesting (`scripts/mutation-floor.sh`, the `mutation-floor` job in
  `.github/workflows/mutation.yml`): a mutation score below the floor fails the
  job. The armed floor is 80% over the authorization resolver
  (`internal/authz`, baseline 0.9375) and the downloadable/credential-scope
  broker (`internal/broker`, baseline 0.849). Remaining follow-up: raise the
  floor toward the canon 60 minimum as the score stabilises (the guards already
  read above it); extend the armed set to the path-containment guard
  (`pathresolver.go`) once a rig-free mutation slice is carved for objectstore;
  and arm the gremlins lens itself only if a de-flaked measurement path
  appears. gremlins stays advisory: it kills on a per-mutant timeout it
  calibrates from one clean run, and the in-scope pgregory.net/rapid property
  suites re-run per mutant blow that budget, so both its dimensions collapse to
  a load-driven number. The go.mod `module`-on-line-1 fix lets gremlins
  attribute coverage correctly (no more phantom 0/0), so its advisory summary
  is meaningful. See the rationale blocks in `.gremlins.yaml` and
  `scripts/mutation-floor.sh`.
- **Handle-store hardening debt.** Two open issues track follow-up work on the
  keystone resolver path (Files-API handle-store, above): #19 (auditgate
  fail-open alignment) and #20 (defense-in-depth hardening). Both are scheduled
  against the north Files-API binding, not this phase.
