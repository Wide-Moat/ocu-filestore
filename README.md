...
The storage broker of Open Computer Use (component-04): the object-store
service — the only door to storage. It custodies the engine's backend
object-store credential and resolves file authorization for the guest session
mount, so neither the guest nor any upstream component ever holds a backend key.
The service mints and signs nothing (invariant 3); it receives the guest's real
filestore credential already validated, stripped, and injected by the Egress
trust-edge, and forwards it to the engine unmodified.
...
storage ADR 0010 (pluggable backend engine), and the frozen wire contracts
under `contracts/storage/`. This repo implements them.
...
- South face: a TLS HTTPS/HTTP-2 REST-JSON listener. The guest reaches it
  outbound through the Egress trust-edge (guest → edge → service direct HTTPS);
  there is no Unix socket. Platform-neutral — no Linux-only socket option
  (see [Platform notes](#platform-notes)).
- Two backend engines: **local-volume** (host filesystem, no network leg) and
  **S3** (network engine, MinIO/Ceph RGW compatible).
...
- Fail-closed hash-chained audit sink (OCSF File System Activity): an
  ...
  restart.
- Graceful shutdown: `SIGTERM`/`SIGINT` → every data-plane listener drains at
  once under a bounded 25s → ops listener shutdown under a bounded 5s, so a
  stop costs at most 30s. The scope on disk is left untouched: its bytes belong
  to the owner, not to the daemon that served them.
...
- Multi-tenant admission: the current shelf admits `trusted_operator` /
  `single-tenant` / `host_local_long_lived` only.
...
|---|---|---|
| South | Session sandbox (guest mount, via [`ocu-rclone-filestore`](https://github.com/Wide-Moat/ocu-rclone-filestore)), reaching the TLS REST listener outbound through the Egress edge | `file-ops.schema.json` + `mount-config.schema.json` |
...
client — the only component that speaks the backend protocol. The engine
enforces `filesystem_id` scope on the edge-injected credential (a foreign scope
is `403`, a missing or expired credential is `401`); the service never
JWKS-verifies the bearer — the edge owns weak-JWT validation. The platform is
...
The south face is a TLS HTTPS/HTTP-2 REST-JSON listener and is **platform
neutral** — it depends on no Linux-only socket option. The guest reaches it
outbound through the Egress edge rather than over a host Unix socket. The
live-S3 and real-binary e2e legs still loud-skip without the MinIO rig; use
`make e2e-linux` to run the full containerised slice. See
...
`stop_grace_period` is 35 s — above the daemon's 30 s worst-case stop (a 25 s
concurrent drain, then a 5 s ops-listener shutdown) — so an in-flight operation
always gets its full drain window before SIGKILL. Nothing else is scheduled
behind the drain: stopping the daemon does not remove the scope, and starting it
again re-serves the same bytes. See
...
| [docs/engines.md](docs/engines.md) | Local-volume vs S3 engine selection, IAM policy, MinIO/RGW path-style, backend credential intake |
