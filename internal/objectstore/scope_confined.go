...
// WriteStream guards the scope and then passes the inner engine's result
// through UNCHANGED, digest included: the confinement decides whether the write
// runs at all, never what it reports. A refused write returns the empty digest
// the interface specifies for an error return (D6).
