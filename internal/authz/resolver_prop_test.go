...
func TestPropPreviewNotDownloadable(t *testing.T) {
	rapid.Check(t, func(rt *rapid.T) {
		scope := FilesystemID(rapid.String().Draw(rt, "scope"))
		ev := CallerEvidence{
			Scope:          scope,
...
// tag (AUTHZ-01, NFR-SEC-43).
func TestPropScopeHintNoWiden(t *testing.T) {
	rapid.Check(t, func(rt *rapid.T) {
		evidenceScope := FilesystemID(rapid.String().Draw(rt, "ev_scope"))
		requestScope := FilesystemID(rapid.String().Draw(rt, "req_scope"))
		if evidenceScope == requestScope {
			return // only the mismatch case is under test
		}
